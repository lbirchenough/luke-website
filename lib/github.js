...
export async function fetchResumeFromGitHub() {
  ...
  try {
    const response = await fetch(url, { 
      headers,
      // Cache for 5 minutes (300 seconds) before revalidating
      next: { revalidate: 300 },
      //cache: 'no-store'
    });
...
export async function fetchResumeHTMLFromGitHub() {
  const url = 'https://api.github.com/repos/lbirchenough/Resume/contents/resume.html';
...
export async function fetchResumePDFFromGitHub() {
  const url = 'https://api.github.com/repos/lbirchenough/Resume/contents/resume.pdf';
